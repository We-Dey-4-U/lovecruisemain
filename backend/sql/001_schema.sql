...
-- ============================================================
-- Follow-to-Live-Room: presence status machine + privacy flags
-- ============================================================

-- Privacy controls (Settings screen: "Allow followers to know
-- when I'm watching Live" / "Allow followers to join the room
-- I'm in" / "Hide my viewing activity")
ALTER TABLE users
...
-- user_presence already exists (used by liveRoomController.join/leave
-- and stream.socket.js). Extend it with the richer status machine.
ALTER TABLE user_presence
...
-- Guard the CHECK / UNIQUE constraints so re-running this migration
-- doesn't blow up if they already exist.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'user_presence_status_check'
  ) THEN
    ALTER TABLE user_presence
      ADD CONSTRAINT user_presence_status_check
      CHECK (status IN ('OFFLINE','ONLINE','WATCHING_LIVE','HOSTING_LIVE','CO_HOST','GUEST_SEAT'));
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'user_presence_user_id_unique'
  ) THEN
    ALTER TABLE user_presence
...
CREATE INDEX IF NOT EXISTS idx_user_presence_current_room ON user_presence(current_room_id);
CREATE INDEX IF NOT EXISTS idx_user_presence_host_id ON user_presence(host_id);
